var lsads = lsads || {};
var ProgramTable = lsads.SelectProgramTable || {};
...
ProgramTable.loadTables = function (startPage, pageSize) {
    ...
    $("#" + ProgramTable.tableBodyId).load("/Program/SelectPageListPartialView?startPage=" + startPage + "&pageSize=" + pageSize + "&_t=" + new Date().getTime(), function (response, status, xhr) {
        if (status == "success") {
            var html = $(response);
